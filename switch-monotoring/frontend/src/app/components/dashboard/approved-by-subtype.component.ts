...
export class ApprovedBySubtypeComponent implements OnChanges {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          footer: (ctxArr: any[]) => {
            ...
            return total > 0 ? `Total approuvées : ${total} tx` : '';
          }
        }
      }
    }
  ...
  private updateChart(): void {
    ...
    // Toutes les transactions APPROUVÉES, tous canaux confondus
    const approved = this.transactions.filter(tx => (tx.status || '').toUpperCase() === 'APPROVED');
